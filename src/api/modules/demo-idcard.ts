 ...
 */
export function mockIdCardOcr(side: 'front' | 'back'): IdCardOcr {
  return side === 'front'
    ? {
        side: 'front',
        ocrStatus: 'success',
 ...
 */
export function mockIdCardUploadResponse(
  side: 'front' | 'back',
  fileUrl = 'https://demo.example.com/idcard.png',
): IdCardUploadResponse {
  return { code: 200, data: { ...mockIdCardOcr(side), url: fileUrl }, message: '识别成功' }
}

/** 身份证上传 + OCR 合并接口请求参数 */
export interface IdCardUploadParams {
  /** 文件名 */
  fileName: string
 ...
 * 上传落盘 → 一次返回图片 url 与全部识别字段（结构见 IdCardOcr）。
 ...
 */
export async function idCardUpload(
  data: IdCardUploadParams,
): Promise<IdCardOcr & { url?: string }> {
  const { post } = await import('../request')
  return post<IdCardOcr & { url?: string }>('/demo/id-card/upload', data as Record<string, any>)
}
 ...
 */
export async function idCardUploadAlt(data: IdCardUploadParams): Promise<Record<string, any>> {
  const { post } = await import('../request')
